...
/**
 * une liste d'éléments, qui contient des doublons Trouver les éléments uniques , eliminer les doublons
 */
...
public class RemoveDoublonOfList {
    ...
    public static List<Integer> uniqueElementOfIntegerListElement2(List<Integer> list) {
        ...
        /*
         * parcourir la liste en se souvenant du dernier élément parcouru. Si l'élément actuel est le même que l'élément
         * précédent, alors c'est un doublon et on ne l'inclut pas dans la liste des éléments uniques.
         */
        if (list != null & !list.isEmpty()) {
            for (Integer element : list) {
                if (!copyList.contains(element)) {
